...
> ⚠️ **使用声明（务必遵守）**
> 本项目仅供个人学习与研究使用。**严禁在社交平台（微博、微信、QQ、Telegram、贴吧、X / Twitter、Reddit、Discord 等）或任何公开 / 半公开渠道提及、宣传、分享、转发本项目，以及其部署地址、源码与配置。** 请低调自用，切勿公开传播。
...
- 导入支持三种来源（优先级从高到低）：① **订阅链接（URL）**——直接填 `https://.../jingjian.txt` 这类 DecoTV 订阅 txt 链接，后端自动抓取；② **上传 .txt 文件**；③ **粘贴 txt / JSON** 文本。
- 🔞 **导入为成人资源**：勾选后，本次导入的全部源会被强制标记为成人（`adult=1`）；不勾选则沿用各源 `is_adult` 字段。
...
- **资源管理默认为空**：全新安装**不再**自动灌源，给你一个干净的起点；
- 想要内置源时二选一：后台「**导入 txt 源**」里贴 `jingjian.txt` 的链接 / 文本，或直接点「**恢复内置默认源**」一键灌入 / 更新。
...
后台「**资源管理**」顶部有一枚开关：**成人内容（🔞）前台显示：开/关**。

- **关**（默认）：前台（首页搜索、源下拉框）自动过滤掉所有成人源，但成人源仍保留在后台、可单独编辑/删除；
- **开**：前台正常展示成人源。
- 该开关写入配置 `show_adult`，对所有访客即时生效；它只控制"前台可见性"，不会删除任何源。
